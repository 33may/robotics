...
Migrated from vbti.logic.cameras.d405_still (2026-08-19) — inspection is
self-contained, no vbti imports.
...
def t_flange_cam() -> np.ndarray:
    """Calibrated T_flange_cam, OpenCV convention. Single source of truth
    is the loader in cell/geometry (the frame belongs to the world model)."""
    from inspection.cell.geometry import load_T_flange_cam
    return load_T_flange_cam("cv")

# 848x480 is the D405's optimal depth resolution (per Intel tuning guide).
WIDTH, HEIGHT, FPS = 848, 480, 15


def open_camera(serial: str = WRIST_SERIAL, width: int = WIDTH,
...
def live(serial: str = WRIST_SERIAL) -> None:
    """Live RGB viewer, sized for the monitor. q quits."""
    import cv2

    pipe, profile, align, depth_scale = open_camera(serial)
    cv2.namedWindow("wrist D405  (q quits)", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("wrist D405  (q quits)", 1400, 790)
    try:
        while True:
            fs = pipe.wait_for_frames(timeout_ms=2000)
            rgb = np.asanyarray(fs.get_color_frame().get_data())
            cv2.imshow("wrist D405  (q quits)", cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR))
