...
Usage:
    python -m vbti.utils.datasets.convert_hdf5_to_lerobot \
        --hdf5 /path/to/dataset.hdf5 \
        --repo-id may33/my_dataset \
        --task "Pick up the duck and place it in the cup" \
        --camera-map cam_top:top cam_left:left cam_right:right wrist:gripper
...
def parse_camera_map(pairs: list[str]) -> dict[str, str]:
    """Parse 'hdf5_key:lerobot_name' pairs into a dict."""
    result = {}
    for pair in pairs:
        if ":" not in pair:
            raise ValueError(f"Camera map entry must be 'hdf5_key:lerobot_name', got '{pair}'")
        hdf5_key, lerobot_name = pair.split(":", 1)
        result[hdf5_key] = lerobot_name
    return result


def main():
    parser = argparse.ArgumentParser(description="Convert leisaac HDF5 to LeRobot v3.0")
    ...
    parser.add_argument("--camera-map", nargs="+", required=True,
                        help="Camera mapping as hdf5_key:lerobot_name pairs")
    parser.add_argument("--root", default=None, help="Output directory (default: HF_LEROBOT_HOME/repo_id)")
    ...
    parser.add_argument("--discover-cameras", action="store_true",
                        help="Print discovered camera keys and exit")
    args = parser.parse_args()

    if args.discover_cameras:
        cameras = discover_cameras(args.hdf5)
