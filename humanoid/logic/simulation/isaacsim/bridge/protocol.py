"""
protocol.py — IPC wire format between the Py 3.8 sidecar and Py 3.11 Isaac driver.

Three message types travel a single AF_UNIX SEQPACKET socket:

  - HELLO       driver → sidecar    once at startup, carries Isaac DOF names
  - CMD         sidecar → driver    per-cmd, mirrors limxsdk RobotCmd (minus motor_names)
  - STATE_IMU   driver → sidecar    per physics tick, articulation state + IMU

All payloads are little-endian fixed-size struct layouts. The same Python source
runs in CPython 3.8.18 (limx env) and CPython 3.8.18-or-newer / 3.11.14 (isaac
env) and produces byte-identical output for any input — `struct.pack` is the
canonical layer for this contract.
...
References:
  - Design: `humanoid/openspec/changes/may-147-isaac-limx-sdk-bridge/design.md` D3, D4, D13
  - Spec: spec.md "IPC protocol is fixed-size, versioned, and Py3.8/3.11 byte-identical"
